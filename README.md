...
class ModernAttendanceSystem:
    ...
    def export_student_to_word(self, student_info, attendance_records):
        """تصدير بيانات الطالب إلى ملف Word مع إضافة جميع أنواع الحضور والغياب
        ...
        try:
            ...
            # إضافة عنوان المستند
            title = doc.add_heading('تقرير بيانات الطالب', level=0)
            title.alignment = WD_ALIGN_PARAGRAPH.CENTER
            for run in title.runs:
                run.font.size = Pt(18)
                run.font.bold = True
            ...
            row = table.add_row().cells
            row[0].text = "مستبعد" if is_excluded == 1 else "موجود"
            row[1].text = 'حالة الطالب'
            ...
            if export_file:
                doc.save(export_file)
                messagebox.showinfo("نجاح", f"تم تصدير بيانات الطالب بنجاح إلى:\n{export_file}")
    ...
    def export_section_to_word(self, course_name, section_name):
        ...
        try:
            ...
            # الحصول على بيانات الطلاب في الفصل المحدد (فقط غير المستبعدين)
            cursor = self.conn.cursor()
            cursor.execute("""
            ...
            if not students_data:
                messagebox.showinfo("ملاحظة",
                                    f"لا يوجد طلاب نشطين مسجلين في فصل '{section_name}' من دورة '{course_name}'")
                return
            ...
            header_para = header.paragraphs[0]
            header_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
            header_run = header_para.add_run(f'كشف حضور وغياب طلاب فصل: {section_name} - دورة: {course_name}')
            header_run.font.size = Pt(14)
            header_run.font.bold = True
            header_run.font.rtl = True
    ...
    def export_section_diligence_behavior(self, course_name, section_name):
        """وظيفة تصدير بيان المواظبة والسلوك للفصل المحدد بتنسيق Word مع ترتيب الطلاب حسب الدرجة"""
        if not self.current_user["permissions"]["can_export_data"]:
            messagebox.showwarning("تنبيه", "ليس لديك صلاحية تصدير البيانات")
            return
        ...
        try:
            ...
            # إضافة عنوان المستند
            title = doc.add_heading(f'بيان المواظبة والسلوك لطلاب فصل: {section_name} - دورة: {course_name}', level=0)
            title.alignment = WD_ALIGN_PARAGRAPH.CENTER
            for run in title.runs:
                run.font.size = Pt(16)
                run.font.bold = True
